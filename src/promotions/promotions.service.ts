...
export class PromotionsService {
  ...
  async getPromotionsForProduct(
    ...
    try {
      ...
      console.error(
        `Błąd podczas pobierania promocji z Fake Store API:`,
        error.message
      );
      return [];
    }
  ...
  async getPromotionsForList(
    ...
    try {
      ...
      const allPromotions = await Promise.all(
        items.map((item) =>
          this.getPromotionsForProduct(item.name, item.category)
        )
      );
      ...
      console.error(
        `Błąd podczas pobierania promocji dla listy:`,
        error.message
      );
